...
if yes > no{
    print("There is a coffee vending machine!")
}
else{
    print("better luck next year")
}
...
func voteProcessing(issue: String, vote: [Bool]) {
    var yes = 0
    var no = 0
    for result in vote{
        if result == true  {
...
voteProcessing(issue: "Should we change the mascot", vote: shouldMascotChangeVotes)
